...
import { FriendlyError, MSG, friendlyError } from './errors';
import { requireSupabase } from './supabase';

export { FriendlyError, MSG, friendlyError, isAuthExpiredError, isNetworkError } from './errors';

type Result<T> = { data: T | null; error: unknown; status?: number };

/** Return a `{ data, error }` result's data, or throw a FriendlyError. */
export function unwrap<T>(res: Result<T>): T {
  if (res.error) {
    // PostgREST reports transport failures as status 0 with an empty code.
...
export async function invokeFunction<T>(name: string, body: Record<string, unknown>): Promise<T> {
  ...
  if (res.error) {
    ...
    throw new FriendlyError(friendlyError(res.error));
  }
  return res.data as T;
}
